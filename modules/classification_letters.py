...
def blob_side_from_pct(hole_count: int, hole_pct: float, split: float = 23.5 ) -> str: #float was 33
    print(f"hole_pct={hole_pct}")
    if hole_count >= 2:
        return "B"
    return "DOQ" if hole_pct >= split else "ABPR"

def classify_blob_branch_cramm(A01, skel01, blob_split=23.5, sym_thresh=0.80): #float was 33
    ...
    if side == "ABPR":
        if e >= 4:
            ...
            if south > north and (south - north) > 0.15:
                return "W"
            else:
                return "M"
        if e <= 1:
        ...
        return "R" if sym <= sym_thresh and e == 2 else "A"
...
def classify_no_blob_cramm(A01, skel01,
    ...
    if e >= 5:
        ...
        # W: usually not top-dominant concavity
        return "W"
            ...
            if vstroke == 1:
                if hstroke >= 1:
                    ...
                    if hs >= 0.95:
                        ...
                        if east > 0.65 and east > west + 0.05 and bw < 0.97:
                            return "G"
