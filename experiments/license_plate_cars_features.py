...
def recognize_plate(img_or_path, debug=False):
    ...
    for idx, char_img in enumerate(normalized_chars):
        ...
        # keep preprocess visualization off for batch evaluation
        ...
        # common fields for every character row
        row = {
            "pos": idx,
            "char_type": ctype,
            "pred_char": None,
        ...
        if ctype == "digit":
            ...
            row["pred_char"] = pred
            row["digit_group"] = group

            # Optional digit-side features
            try:
                blobs_img, n_blobs = find_blobs(A)
                row["n_blobs"] = int(n_blobs)

                if n_blobs > 0:
                    stems_img, n_stems, _ = get_stems(A, blobs_img)
                    row["n_stems"] = int(n_stems)
                else:
                    row["n_stems"] = 0
            except Exception:
                row["n_blobs"] = None
            ...
            row["pred_char"] = pred
            row["endpoints"] = int(e)
            row["vstrokes"] = int(vstrokes)
            row["hstrokes"] = int(hstrokes)
            row["hole_count"] = int(hc)
            row["hole_pct"] = float(hpct)
            row["vsym"] = float(vs)
            row["hsym"] = float(hs)
            row["north"] = float(north)
            row["south"] = float(south)
            row["west"] = float(west)
            row["east"] = float(east)
            row["bottom_width_ratio"] = float(bw)
            row["center_density_ratio"] = float(ratio)
            row["left_open_ratio"] = float(left_ratio)
            row["right_open_ratio"] = float(right_ratio)
    ...
    return {
        ...
        "chars": char_debug_rows,
        "n_boxes": len(boxes),
        "n_normalized": len(normalized_chars),
        "plate_box": plate_box,
    }
