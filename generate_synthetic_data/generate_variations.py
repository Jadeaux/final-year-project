...
def add_salt_pepper(img, amount=0.003):
    out = img.copy()
    h, w = out.shape[:2]
    n = int(amount * h * w)
...
def jpeg_recompress(img, q_min=40, q_max=95):
    q = int(np.random.uniform(q_min, q_max))
...
def make_variation_centered(base):
    ...
    if np.random.rand() < 0.5:
        img = add_salt_pepper(img, amount=np.random.uniform(0.001, 0.008))

    if np.random.rand() < 0.8:
        img = jpeg_recompress(img, q_min=40, q_max=95)
...
def generate_all_letters(templates_dir="images/letters",
                         out_root="images/variations",
                         n=200):
    ensure_dir(out_root)

    # grabs letterA.png, letterB.png, ... (also works for digits if named letter0.png etc)
    paths = sorted(glob.glob(os.path.join(templates_dir, "letter*.png")))
    if not paths:
...
if __name__ == "__main__":
    ...
    #     templates_dir="images/letters",
    #     out_root="images/variations",
    #     n=200
    # )
    generate_variations_for_image(
        input_path=r"C:\Users\jadsa\GitRepos\final-year-project-jad\images\letters/letterA.png",
        out_dir=r"C:\Users\jadsa\GitRepos\final-year-project-jad\images\variations/A",
        n=200
    )