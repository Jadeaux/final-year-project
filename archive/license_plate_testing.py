...
IMG_PATH = "images/plates/image.png"
...
y_start = int(0.40 * H)
y_end   = int(0.75 * H)
mask_roi[y_start:y_end, x_start:x_end] = 255
bw = cv2.bitwise_and(bw, mask_roi)

# 4) Gentle horizontal linking only
k_close = cv2.getStructuringElement(cv2.MORPH_RECT, (17, 3))
...
for c in contours:
    ...
    print("\nCandidate:", (x, y, w, h), f"ar={ar:.2f}", f"area={area}")
    ...
    if y < 0.42 * H or y > 0.70 * H:
        print("  reject: y-band")
        continue

    roi = bw[y:y+h, x:x+w]
    fg_ratio = np.count_nonzero(roi) / float(w * h)
    print(f"  fg_ratio={fg_ratio:.3f}")

    # allow text-like density
    if fg_ratio < 0.08 or fg_ratio > 0.90:
        print("  reject: fg_ratio")
        continue
    ...
    # prefer good rectangle shape around ~3:1 to ~4:1
    ar_target = 3.5
    ar_penalty = abs(ar - ar_target)

    # final score: wide, central, plate-like aspect ratio
    score = (w * h) / (1.0 + 4.0 * center_penalty + 1.5 * ar_penalty)
...
if best is None:
    raise RuntimeError("No plate chosen")
else:
    x, y, w, h = best
    print("CHOSEN PLATE:", best)

    x, y, w, h = pad_bbox(x, y, w, h, H, W, pad_x_frac=0.35, pad_y_frac=0.45)
...
cv2.imshow("chars_mask_initial", chars)
